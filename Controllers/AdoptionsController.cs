...
namespace mvc_pets.Controllers
{
    public class AdoptionsController : BaseController
    {
        ...
        public AdoptionsController(ApplicationDbContext context, ILogger<AdoptionsController> logger, UserManager<ApplicationUser> userManager)
            : base(context)
        {
            _logger = logger;
        ...
        public async Task<IActionResult> MyRequests()
        {
            try
            {
                ...
                if (!requests.Any())
                {
                    TempData["InfoMessage"] = "You haven't made any adoption requests yet. Browse available pets to find your perfect companion!";
                    return RedirectToAction("Index", "Pets");
                }
            ...
            {
                _logger.LogError($"Error loading adoption requests: {ex.Message}");
                TempData["ErrorMessage"] = "An error occurred while loading your adoption requests.";
                return View(new List<Adoptions>());
            }
        }
